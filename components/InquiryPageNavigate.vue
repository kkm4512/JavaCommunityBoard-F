<template>
  <nav aria-label="Page navigation example" v-if="props.datas.totalPages >= 1">
    <ul class="inline-flex -space-x-px text-base h-10">
      <li>
        <NuxtLink
          :to="`/inquiry/${Math.max(props.datas.curPage - 1, 1)}/getInquiry`"
      ...
      <li>
        <NuxtLink
          :to="`/inquiry/${Math.min(props.datas.curPage + 1, props.datas.totalPages)}/getInquiry`"
