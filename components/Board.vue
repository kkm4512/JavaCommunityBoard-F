<template>
  ...
  >
    <div class="bg-gray-300 rounded-xl p-3">
      ...
      <div v-if="removeModalOpen" class="text-black">
        <BoardRemoveModal :removeModalOpen="removeModalOpen" @modalClicked="handleShareRemoveModalClicked" />
      </div>
      ...
      <h5 class="flex justify-center mb-5 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ props.board.title }}</h5>
      <div class="flex justify-between text-sm"></div>
      <div class="flex justify-center">
        <div v-if="boardImageUrlPath" class="max-w-[50%] max-h-[50%]">
          <img :src="boardImageUrlPath" />
      ...
      <p class="flex justify-center font-normal text-gray-700 dark:text-gray-400 mb-5 mt-5">{{ props.board.description }}</p>
...
async function handleUpdate() {
  ...
  updateModal.value.updateBoardImage = props.board.boardImagePath;
}
...
async function handleShareRemoveModalClicked(option?: string) {
  removeModalOpen.value = !removeModalOpen.value;
  const data = {
    boardId: props.board.boardId,
...
async function handleShare(board: ResponseBoard) {
  ...
  const saved = await boardFetch("/share", "POST", data);
